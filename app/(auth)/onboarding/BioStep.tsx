...
export default function BioStep({ data, updateData, onNext, onBack }: BioStepProps) {
  ...
  const lookingForWordCount = data.lookingFor.trim().split(/\s+/).filter(Boolean).length;
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div>
          <Label htmlFor="bio" className="mb-2 block">About You *</Label>
          <Textarea
            id="bio"
            value={data.bio}
          ...
          <div className="flex justify-between items-center mt-1">
            <p className={`text-sm ${
              bioWordCount < 50 || bioWordCount > 300 ? "text-red-500" : "text-gray-600"
            }`}>
        ...
        <div>
          <Label htmlFor="lookingFor" className="mb-2 block">What You're Looking For *</Label>
          <Textarea
            id="lookingFor"
          ...
          <div className="flex justify-between items-center mt-1">
            <p className={`text-sm ${
              lookingForWordCount < 20 || lookingForWordCount > 100 ? "text-red-500" : "text-gray-600"
            }`}>
              {lookingForWordCount} / 20-100 words
            </p>
          </div>
          {errors.lookingFor && <p className="text-sm text-red-500 mt-1">{errors.lookingFor}</p>}
        </div>
      </div>
